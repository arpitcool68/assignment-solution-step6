...
public class LoggerAspect {
	...
	@AfterReturning(value = "execution(* com.stackroute.user.controller.UserAuthController.*.*(..))", returning = "result")
	public void afterReturning(JoinPoint joinPoint, Object result) {
		LOG.info("{} returned with value {}", joinPoint, result);
	}

	@After(value = "execution(* com.stackroute.user.controller.UserAuthController.*.*(..))")
	public void after(JoinPoint joinPoint) {
		LOG.info("after execution of {}", joinPoint);
	}

	@AfterThrowing(value = "execution(* com.stackroute.user.controller.UserAuthController.*.*(..))")
